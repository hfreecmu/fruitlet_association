...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, name: str, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        self.name = name
        if not self.name in ["self", "cross"]:
    ...
    def forward(
        ...
        if not src0.dim() == 3:
            why_not_sparsity_fast_path = f"input not batched; expected src.dim() of 3 but got {src0.dim()}"
        elif self.training:
        ...
        elif src0.is_nested and (src_key_padding_mask is not None or src_mask is not None):
        ...
        if self.norm_first:
            if self.name == "self":
                ...
                raise RuntimeError("Illegal name: " + self.name)

            src0 = src0 + self._ff_block(self.norm2(src0))
            ...
            src0 = self.norm2(src0 + self._ff_block(src0))
            src1 = self.norm2(src1 + self._ff_block(src1))
